...
void register_registers()
{
    ...
    for (auto& dyn_reg : dyn_regs)
        register_manager.add_register(dyn_reg.name, std::make_unique<DynamicRegister>(dyn_reg.func));

    for (size_t i = 0; i < 10; ++i)
    {
        register_manager.add_register('0'+i, std::make_unique<DynamicRegister>(
